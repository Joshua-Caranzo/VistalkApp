<script lang="ts">
  ...
  onMount(async () => {
    searchQueries.forEach((query, index) => {
      const matchingChoice = choices.find(
        (choice) =>
          choice.englishTranslation.toLowerCase() === query.toLowerCase(),
      );
      if (matchingChoice) {
  ...
  function selectChoice(choice: Content, index: number) {
    if (selectedChoices) selectedChoices[index] = choice;
              ...
              {#if fileType}
                <div class="mt-5">
                  <div class="flex items-justify">
                    <label
                      for="file"
                      class="flex mr-2 bg-black text-white text-sm p-1 rounded-md cursor-pointer"
                      >Upload File
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="1.5em"
            ...
            <div class="mt-4 relative flex items-center">
              ...
              <div class="relative w-full">
                ...
                {#if searchQueries[i - 1].length > 0 && !selectedChoices[i - 1]}
                  <ul
                    class="absolute left-2 w-full bg-white border border-gray-200 rounded-md shadow-lg mt-1 z-10 top-full"
                  >
