...
<div class="flex flex-col sm:flex-row justify-between items-center mt-1 bg-white rounded-xl py-4 px-4 shadow-lg">
    ...
    <div class="flex flex-col sm:flex-row gap-4 mt-4 sm:mt-0">
        <select bind:value={itemTypeID} on:change={refresh}  class="w-full sm:w-auto font-['Helvetica'] bg-white text-black py-2 px-3 rounded-xl text-sm shadow-lg">
                    {#each itemTypes as item}
                        <option class="py-2" value={item.itemTypeID}>{item.typeName}</option>
                    {/each}
                </select>
        <button on:click={() => modalOpen(true, null , null, null)} class="flex items-center font-['Helvetica'] bg-white text-black py-2 px-3 rounded-xl text-sm shadow-lg hover:bg-[#6addd0] transform hover:scale-110 transition-transform duration-300">
    ...
    <div class="mt-6 overflow-x-auto">
        <table class="bg-white w-full shadow-lg rounded-xl min-w-[640px]">
            <thead class="font-['Cambria'] bg-gradient-to-r from-[#6addd0] to-[#f7c188] text-white text-center">
                <tr>
                    <th class="px-4 py-2 first:rounded-tl-xl last:rounded-tr-xl">Title</th>
        ...
        <table class="bg-white w-full shadow-lg rounded-xl min-w-[640px]">
            ...
            <tbody class="text-center text-sm">
                ...
                {#each coinBagList as coinBag}
                    <tr>
                        <td class="px-4 py-2 first:rounded-tl-xl last:rounded-tr-xl">{coinBag.coinBagName}</td>
                        <td class="px-4 py-2">{coinBag.quantity}</td>
                        <td class="px-4 py-2">{coinBag.moneyPrice}</td>
                        <td class="px-4 py-2 first:rounded-tl-xl last:rounded-tr-xl">
