...
{:else}
    <button class="bg-black rounded-lg p-1" on:click={goBackToSection}
        ><svg
        ...
        <table class="bg-white w-full shadow-lg rounded-xl min-w-[640px]">
            ...
            <tbody class="text-center text-sm hover:cursor-pointer">
                ...
                {:else if unitCallResult.totalCount != null && unitCallResult.totalCount > 0}
                    {#each units as u}
                        <tr
                            class="border-t-2 mx-4"
                            on:click={() => openQuestionList(u.unitID)}
                        >
    ...
    {#if unitCallResult.totalCount}
        <Pagination
            totalCount={unitCallResult.totalCount}
            {pageNo}
